...
const StackPool = struct {
    ...
    fn init(min_stack_size: usize, stack_count: usize, cache_len: usize) error{OutOfMemory}!StackPool {
        ...
        const rounded_stack_size = switch (comptime builtin.os.tag) {
            .windows => @max(min_stack_size, 8 * 1024),
            else => 0,
        };
        const stack_size = mem.alignForward(usize, rounded_stack_size + page_size, page_size);
...
const MessageQueue = struct {
    ...
    fn tryPop(self: *MessageQueue) ?*Message {
        // NOTE(gabriel):
        //
        // The executor event loop is the only consumer of messages, therefore the
        // pop list does not require any ordering constraints.
        if (self.pop_list.load(.monotonic)) |msg| {
            self.pop_list.store(msg.next, .monotonic);
            msg.next = null;
            return msg;
        }

        const orig = self.push_list.swap(0, .acquire);
        ...
        self.pop_list.store(pop_list_head, .monotonic);
    ...
    fn pop(self: *MessageQueue, futex: *Futex, timeout: Instant) ?*Message {
        ...
        const orig = self.push_list.swap(0, .acquire);
        if (orig & list_mask == 0) return self.popSlow(futex, timeout);
    ...
    fn popSlow(self: *MessageQueue, futex: *Futex, timeout: Instant) ?*Message {
        ...
        // NOTE(gabriel):
        //
        // Only the consumer (we) can append messages to the pop list.
        // So we don't need to check it.
        var orig = self.push_list.swap(0, .monotonic);
        while (true) {
            if (orig & list_mask != 0) {
                _ = self.push_list.load(.acquire);

                // NOTE(gabriel): Same as in pop.
                var head: *Message = @ptrFromInt(orig & list_mask);
                var pop_list_head: ?*Message = null;
                while (head.next) |next| {
                    head.next = pop_list_head;
            ...
            if (spin_count < spin_limit) {
                if (spin_count < spin_relax_limit)
                    ...
                    Thread.yield() catch {};
                spin_count += 1;
                orig = self.push_list.swap(0, .monotonic);
                continue;
            }

            if (orig & waiting_bit == 0) {
                if (self.push_list.cmpxchgWeak(0, waiting_bit, .monotonic, .monotonic)) |v| {
                    orig = v;
                    continue;
                }
            }
...
const GlobalChannel = struct {
    ...
    const Element = struct {
        value: atomic.Value(*Task) align(atomic.cache_line),
    };
    ...
    fn tryPop(self: *GlobalChannel, seed: usize) error{Closed}!?*Task {
        ...
        const task = slot.value.load(.acquire);
        return task;
    }
...
pub const CmdBuf = struct {
    ...
    fn finish(self: *CmdBuf, futex: *Futex) bool {
        if (self.cmd_buf.deinit) |f| f(self.cmd_buf);
            ...
            switch (msg.tag) {
                ...
                .task_complete, .task_abort => {
                    ...
                    if (self.stack_pool.push(task.stack)) |waiter| {
                        debug.assert(waiter.node.prev == null);
                        debug.assert(waiter.node.next == null);
                        self.process_queue.append(&waiter.node);
                    }
                    self.task_pool.destroy(futex, task);
                    _ = self.task_count.fetchSub(1, .monotonic);

                    // NOTE(gabriel): Enqueue if not already enqueued.
                    if (cmd_buf.node.prev == null and cmd_buf.node.next == null) {
                        self.process_queue.append(&cmd_buf.node);
                    }
                },
                .task_sleep => {
