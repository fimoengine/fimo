...
const HashTable = struct {
    ...
    const Waiter = union(enum) {
        ...
        fn prepare_park(self: *Waiter) void {
            switch (self.*) {
                .thread => |*thread| {
                    thread.futex.store(1, .monotonic);
                },
                .task => |_| {
                    @panic("TODO");
                },
        ...
        fn timed_out(self: *Waiter) bool {
            switch (self.*) {
                .thread => |*thread| {
                    return thread.futex.load(.monotonic) != 0;
                },
                .task => |_| {
                    @panic("TODO");
                },
        ...
        fn park(self: *Waiter, timeout: ?Instant) bool {
            switch (self.*) {
                .thread => |*thread| {
                    while (thread.futex.load(.acquire) != 0) {
                        if (timeout) |t| {
                            ...
                            Futex.timedWait(&thread.futex, 1, @truncate(nanos)) catch return false;
                        } else {
                            Futex.wait(&thread.futex, 1);
                        }
                    }
    ...
    fn lockSlow(self: *WordLock) void {
        ...
        while (true) {
            ...
            var entry = QueueEntry{
                .waiter = if (is_task) @panic("TODO") else .{ .thread = .{} },
            };
...
fn lockBucketChecked(self: *Self, key: *atomic.Value(*const anyopaque)) std.meta.Tuple(&.{
    *const anyopaque,
    *HashTable.Bucket,
}) {
    while (true) {
...
fn lockBucketPair(self: *Self, key_1: *const anyopaque, key_2: *const anyopaque) std.meta.Tuple(&.{
    *HashTable.Bucket,
    *HashTable.Bucket,
}) {
    while (true) {
...
fn createMultiQueueEntry(token: ParkToken) HashTable.MultiQueueEntry {
    ...
    return HashTable.MultiQueueEntry{
        .waiter = waiter,
        .park_token = token,
    };
}
...
pub fn park(
...

    {
        ...
        entry.multi_queue.waiter.prepare_park();
        if (bucket.head) |_| {
    ...
    const unparked = entry.multi_queue.waiter.park(timeout);
    if (unparked) return ParkResult{ .type = .unparked, .token = entry.multi_queue.unpark_token };
    ...
    // Check again for a timeout, since we hold the bucket lock.
    if (!entry.multi_queue.waiter.timed_out())
        return ParkResult{ .type = .unparked, .token = entry.multi_queue.unpark_token };

    // We timed out, so we remove the entry from the queue.
    var link: *?*HashTable.Entry = &bucket.head;
    var current: ?*HashTable.Entry = bucket.head;
    ...
    while (current) |curr| {
        ...
        if (bucket.tail == curr) {
            ...
            var scan = next;
            while (scan) |scan_entry| : (scan = scan_entry.next) {
...
pub fn parkMultiple(
    ...
    for (keys, 0..) |k, i| if (std.mem.count(*const anyopaque, keys[0..i], (&k)[0..1]) != 0)
        return ParkMultipleResult{ .type = .keys_invalid };
    ...
    multi_queue.waiter.prepare_park();
    ...
    for (keys, entries, 0..) |key, *entry, i| {
        ...
        // The check that the entry has not yet been consumed.
        {
            ...
            defer multi_queue.mutex.unlock();
            if (multi_queue.consumer_key != null) {
    ...
    if (enqueue_status != .ok) {
        ...
        switch (enqueue_status) {
            .invalid => return ParkMultipleResult{ .type = .invalid },
            .consumed => {
    ...
    _ = multi_queue.waiter.park(timeout);
    ...
    // Now that we are dequeued we can check again for a timeout.
    if (multi_queue.waiter.timed_out())
        return ParkMultipleResult{ .type = .timed_out };

    // If we did not time out, we must have been unparked.
    return ParkMultipleResult{
        .type = .unparked,
...
pub fn unparkOne(
    ...
    while (current) |curr| {
        ...
        {
            curr.multi_queue.mutex.lock();
            defer curr.multi_queue.mutex.unlock();
            if (curr.multi_queue.consumer_key != null) {
                link = &curr.next;
                previous = curr;
                current = curr.next;
                continue;
            }
            curr.multi_queue.consumer_key = key;
        }
        ...
        if (bucket.tail == curr) {
            ...
            while (scan) |scan_entry| : (scan = scan_entry.next) {
                if (scan_entry.key.load(.monotonic) == key) {
                    scan_entry.multi_queue.mutex.lock();
                    defer scan_entry.multi_queue.mutex.unlock();
                    if (scan_entry.multi_queue.consumer_key != null) continue;
        ...
        curr.multi_queue.unpark_token = callback(callback_data, result);

        // Prevent a timeout from freeing the entry before we can unlock the bucket.
        const handle = curr.multi_queue.waiter.lock_unpark();
        defer handle.unpark();
        bucket.mutex.unlock();
        return result;
    }
...
pub fn unparkAll(self: *Self, key: *const anyopaque, token: UnparkToken) usize {
    ...
    var handles_allocator = std.heap.stackFallback(
        @sizeOf(HashTable.UnparkHandle) * 8,
        self.allocator.allocator(),
    );
    const allocator = handles_allocator.get();
    var handles = std.ArrayListUnmanaged(HashTable.UnparkHandle){};
    ...
    while (current) |curr| {
        ...
        // Set the token for the entry.
        curr.multi_queue.unpark_token = token;

        // Unpark all entries after the bucket is unlocked.
        handles.append(allocator, curr.multi_queue.waiter.lock_unpark()) catch @panic("oom");
        current = next;
    }

    // Unlock the bucket and unpark all waiters.
    bucket.mutex.unlock();
    for (handles.items) |handle| handle.unpark();
...
pub fn unparkFilter(
    ...
    while (current) |curr| {
        ...
        const op: FilterOp = filter(filter_data, curr.multi_queue.park_token);
        switch (op) {
            .unpark => {
                // Remove the entry from the queue.
                curr.multi_queue.consumer_key = key;
                ...
                handles.append(allocator, .{
                    .handle = curr.multi_queue.waiter.lock_unpark(),
                    .entry = curr,
                }) catch @panic("oom");
    ...
    for (handles.items) |pair| pair.entry.multi_queue.unpark_token = token;

    // Unlock the bucket and unpark all waiters.
    bucket.mutex.unlock();
    for (handles.items) |pair| pair.handle.unpark();
    return result;
}
...
pub fn unparkRequeue(
    ...
    // Go through the queue looking for entries with a matching key.
    var link: *?*HashTable.Entry = &bucket_from.head;
    var current: ?*HashTable.Entry = bucket_from.head;
    var previous: ?*HashTable.Entry = null;
    var result = UnparkResult{};
    while (current) |curr| {
        ...
        if (result.unparked_tasks < op.num_tasks_to_unpark) {
            curr.multi_queue.consumer_key = key_from;
