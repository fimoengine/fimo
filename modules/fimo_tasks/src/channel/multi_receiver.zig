...
pub fn MultiReceiver(comptime Receivers: type, comptime T: type) type {
    ...
    return struct {
        ...
        fn recvSlow(self: *const Self, lot: *ParkingLot, timeout: ?Instant) TimedRecvError!T {
            ...
            loop: while (true) {
                ...
                inline for (&receiver_open, self.receivers) |*s, rx| {
                    const result = rx.tryRecv() catch |err| switch (err) {
                        RecvError.Closed => blk: {
                            s.* = false;
...
pub fn multi_receiver(comptime T: type, receivers: anytype) MultiReceiver(@TypeOf(receivers), T) {
...
fn channelTest(
    ...
    for (0..repeats) |_| {
        ...
        const rx = multi_receiver(*Node, .{ channel_1.receiver(), channel_2.receiver() });
