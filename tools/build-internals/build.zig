...
pub const FimoBuild = struct {
    ...
    pub const Module = struct {
        ...
        pub fn getLinkModule(self: *Module) *Build.Module {
            ...
            bytes.appendSlice(
                b.allocator,
                b.fmt("// autogenerated for module {s}\n", .{self.name}),
            ) catch @panic("oom");
            bytes.appendSlice(b.allocator,
                \\const fimo_std = @import("fimo_std");
                \\const modules = fimo_std.modules;
                \\const ModuleBundle = modules.ModuleBundle;
                \\
                \\pub const fimo_module_bundle = ModuleBundle(.{
                \\
            ) catch @panic("oom");
            if (self.root_module != null) {
                bytes.appendSlice(b.allocator, "\t@import(\"module\").fimo_module_bundle,\n") catch @panic("oom");
            }
            for (self.module_deps) |dep| {
                bytes.appendSlice(b.allocator, b.fmt("\t@import(\"{s}\").fimo_module_bundle,\n", .{dep})) catch @panic("oom");
            }
            bytes.appendSlice(b.allocator,
                \\});
                \\
            ...
            root_module.addImport("fimo_std", owner.getPackage("fimo_std").root_module);
