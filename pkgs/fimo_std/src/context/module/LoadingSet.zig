...
const priv_modules = @import("../modules.zig");
...
const LoadGraph = struct {
    ...
    fn init(set: *Self) !*LoadGraph {
        ...
        const allocator = priv_modules.sys.allocator;
    ...
    fn spawnMissingTaks(self: *LoadGraph) !void {
        ...
        check: while (module_it.next()) |entry| {
            ...
            if (priv_modules.sys.getInstance(name) != null) {
                tracing.emitWarnSimple(
            ...
            for (info.status.unloaded.@"export".getSymbolImports()) |imp| {
                ...
                } else if (priv_modules.sys.getSymbolCompatible(imp_name, imp_ns, imp_ver) == null) {
                    tracing.emitWarnSimple(
            ...
            for (info.status.unloaded.@"export".getSymbolExports()) |e| {
                ...
                if (priv_modules.sys.getSymbol(e_name, e_ns) != null) {
                    tracing.emitWarnSimple(
            ...
            const id = try self.dependency_tree.addNode(
                priv_modules.sys.allocator,
                .{ .module = name },
            );
            try self.modules.put(priv_modules.sys.allocator, name, id);
        }
        ...
        connect: while (module_it.next()) |entry| {
            ...
            for (info.status.unloaded.@"export".getSymbolImports()) |imp| {
                ...
                if (set.getSymbol(i_name, i_namespace, i_version)) |sym| {
                    ...
                    _ = try self.dependency_tree.addEdge(priv_modules.sys.allocator, {}, id, owner_id.?);
                }
            }
...
fn addModulesFromHandle(
    ...
    var append_data = AppendModulesData{
        .sys = &priv_modules.sys,
        .filter_fn = filter_fn,
        .filter_data = filter_data,
    };
    defer append_data.exports.deinit(priv_modules.sys.allocator);
...
fn addModulesFromPath(
    ...
    const module_handle = try ModuleHandle.initPath(priv_modules.sys.allocator, path);
...
const LoadOp = FSMFuture(struct {
    ...
    pub fn __state1(self: *@This(), waker: pub_tasks.Waker) void {
        ...
        priv_modules.sys.lock();
        errdefer priv_modules.sys.unlock();
    ...
    pub fn __state3(self: *@This(), waker: pub_tasks.Waker) void {
        ...
        set.unlock();
        self.start_instance_future = inner.start(&priv_modules.sys, &self.err);
    }
    ...
    pub fn __state4(self: *@This(), waker: pub_tasks.Waker) pub_tasks.FSMOp {
        ...
        priv_modules.sys.addInstance(inner) catch |e| {
            if (@errorReturnTrace()) |tr| tracing.emitStackTraceSimple(tr.*, @src());
            tracing.emitWarnSimple(
...
const CommitOp = FSMFuture(struct {
    ...
    fn init(set: *Self) EnqueuedFuture(Fallible(void)) {
        ...
        return tasks.Task.initFuture(@TypeOf(f), &f) catch |e| tasks.initErrorFuture(void, e);
    }
    ...
    pub fn __state0(self: *@This(), waker: pub_tasks.Waker) !pub_tasks.FSMOp {
        priv_modules.sys.lock();
        defer priv_modules.sys.unlock();

        self.set.lock();
        defer self.set.unlock();

        // Ensure that no two commit operations are running in parallel.
        if (priv_modules.sys.state == .loading_set) {
            try priv_modules.sys.loading_set_waiters.append(
                priv_modules.sys.allocator,
                .{ .waiter = self, .waker = waker.ref() },
            );
            return .yield;
        }
        priv_modules.sys.state = .loading_set;
        errdefer priv_modules.sys.state = .idle;
    ...
    pub fn __unwind1(self: *@This(), reason: pub_tasks.FSMUnwindReason) void {
        ...
        priv_modules.sys.state = .idle;
        if (priv_modules.sys.loading_set_waiters.pop()) |waiter| {
