...
pub fn Module(T: type) type {
        ...
        inline for (std.meta.fieldNames(@TypeOf(T.fimo_exports))) |name| {
            ...
            if (@typeInfo(@TypeOf(value)) == .pointer) {
                ...
                const wrapper = struct {
                    fn init(inst: *const modules.OpaqueInstance) anyerror!*symbol.T {
                        _ = inst;
                ...
                builder = builder.withDynamicExportSync(
            ...
            const wrapper = struct {
                const Sync = struct {
                    ...
                    fn poll(this: *@This(), waker: tasks.Waker) tasks.Poll(Result) {
                        ...
                        inline for (std.meta.fieldNames(Args), 0..) |name, i| {
                            switch (@FieldType(Args, name)) {
                                *T => args[i] = &Global.state,
                                modules.LoadingSet => args[i] = this.set,
                                else => @compileError("fimo: invalid init event, got invalid parameter type, found: " ++ @typeName(@FieldType(Args, name))),
                            }
                        }
                ...
                fn deinit(inst: *const modules.OpaqueInstance) void {
                    _ = inst;
                    if (@hasField(@TypeOf(T.fimo_events), "deinit")) T.fimo_events.deinit();
                    ctx.Handle.unregisterHandle();
                    Global.instance = undefined;
                    Global.is_init = false;
                }
    ...
    return struct {
        ...
        pub fn queryNamespace(
            ...
            return switch (try instance().queryNamespace(namespace, err)) {
                .removed => .removed,
                .added => .added,
                .static => .static,
            };
        ...
        pub fn addNamespace(namespace: [:0]const u8, err: *?AnyError) AnyError.Error!void {
            try instance().addNamespace(namespace, err);
        }
        ...
        pub fn removeNamespace(namespace: [:0]const u8, err: *?AnyError) AnyError.Error!void {
            try instance().removeNamespace(namespace, err);
        }
        ...
        pub fn queryDependency(
            dep: *const modules.Info,
            err: *?AnyError,
        ) AnyError.Error!enum { removed, added, static } {
            return switch (try instance().queryDependency(dep, err)) {
                .removed => .removed,
                .added => .added,
                .static => .static,
            };
        ...
        pub fn addDependency(dep: *const modules.Info, err: *?AnyError) AnyError.Error!void {
            try instance().addDependency(dep, err);
        }
        ...
        pub fn removeDependency(dep: *const modules.Info, err: *?AnyError) AnyError.Error!void {
            try instance().removeDependency(dep, err);
        }
        ...
        pub fn loadSymbolGroup(
            ...
            err: *?AnyError,
        ) AnyError.Error!modules.SymbolGroup(symbols) {
            return try instance().loadSymbolGroup(symbols, err);
        }
        ...
        pub fn loadSymbol(
            comptime symbol: modules.Symbol,
            err: *?AnyError,
        ) AnyError.Error!modules.SymbolWrapper(symbol) {
            return try instance().loadSymbol(symbol, err);
        }
        ...
        pub fn loadSymbolRaw(
            ...
            return try instance().loadSymbolRaw(name, namespace, version, err);
        }
        ...
        pub fn readParameter(
            ...
            err: *?AnyError,
        ) AnyError.Error!ParamT {
            return try instance().readParameter(ParamT, module, parameter, err);
        }
        ...
        pub fn writeParameter(
            ...
            try instance().writeParameter(ParamT, value, module, parameter, err);
        }
    };
}
