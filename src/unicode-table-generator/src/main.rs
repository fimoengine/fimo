...
fn main() {
    let src_write_location = std::env::args().nth(1).unwrap_or_else(|| {
        ...
        eprintln!(
            "e.g. {} ffi_library/fimo_std/src/impl/unicode.c",
    ...
    let header_write_location = std::env::args().nth(2).unwrap_or_else(|| {
        ...
        eprintln!(
            "e.g. {} ffi_library/fimo_std/include/fimo_std/impl/unicode.h",
            std::env::args().next().unwrap_or_default()
        );
        std::process::exit(1);
    });
    ...
    table_file.push_str("#define FIMO_IMPL_EXPOSE_UNICODE\n");
    table_file.push_str("#include <fimo_std/impl/unicode.h>\n");
    ...
    header_file.push_str("#ifndef FIMO_IMPL_UNICODE_H\n");
    header_file.push_str("#define FIMO_IMPL_UNICODE_H\n");
    ...
    header_file.push_str("#ifdef FIMO_IMPL_EXPOSE_UNICODE\n");
    header_file.push('\n');
    ...
    header_file.push_str("#endif // FIMO_IMPL_UNICODE_H\n");
...
fn version() -> String {
    ...
    out.push_str(&format!(
        "#define FIMO_IMPL_UNICODE_VERSION_MAJOR {major}\n"
    ));
    out.push_str(&format!(
        "#define FIMO_IMPL_UNICODE_VERSION_MINOR {minor}\n"
    ));
    out.push_str(&format!(
        "#define FIMO_IMPL_UNICODE_VERSION_UPDATE {micro}\n"
    ));
...
fn generate_tests(ranges: &[(&str, Vec<Range<u32>>)]) -> String {
    ...
    s.push_str("#define FIMO_IMPL_EXPOSE_UNICODE\n");
    s.push_str("#include <fimo_std/char.h>\n");
    s.push_str("#include <fimo_std/impl/unicode.h>\n");
    s.push('\n');
...
fn generate_asserts(s: &mut String, property: &str, points: &[u32], truthy: bool) {
    for range in ranges_from_set(points) {
        if range.end == range.start + 1 {
            s.push_str(&format!(
                "    assert_true({}fimo_impl_unicode_{}_lookup((FimoChar){}));\n",
                if truthy { "" } else { "!" },
                property.to_lowercase(),
                range.start,
            ));
            ...
            s.push_str(&format!(
                "        assert_true({}fimo_impl_unicode_{}_lookup(ch));\n",
                if truthy { "" } else { "!" },
                property.to_lowercase(),
            ));
            s.push_str("    }\n");
