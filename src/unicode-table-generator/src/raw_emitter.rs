...
impl RawEmitter {
    ...
    fn blank_line(&mut self) {
        if self.src_file.is_empty() || self.src_file.ends_with("\n\n") {
            return;
        }
        writeln!(&mut self.src_file).unwrap();
    }

    fn emit_bitset(&mut self, ranges: &[Range<u32>], property: &str) -> Result<(), String> {
        ...
        for length in 1..=64 {
            ...
            temp.emit_chunk_map(word_indices[&0], &compressed_words, length, property);
        ...
        writeln!(
            &mut self.src_file,
            "static const FimoU64 {}_BITSET_CANONICAL[{}] = {{{}}};",
            property.to_uppercase(),
            canonicalized.canonical_words.len(),
            fmt_list(canonicalized.canonical_words.iter().map(|v| Hex(*v))),
        )
        .unwrap();
        self.bytes_used += 8 * canonicalized.canonical_words.len();
        writeln!(
            &mut self.src_file,
            "static const FimoU8 {}_BITSET_MAPPING[{}][2] = {{{}}};",
            property.to_uppercase(),
        ...
        #[rustfmt::skip]
        let mut x = || {
            let uppercase = property.to_uppercase();
            let lowercase = property.to_lowercase();
            ...
            writeln!(&mut self.src_file, "        {}_BITSET_CHUNKS_MAP,", uppercase).unwrap();
            writeln!(&mut self.src_file, "        sizeof({0}_BITSET_CHUNKS_MAP)/sizeof({0}_BITSET_CHUNKS_MAP[0]),", uppercase).unwrap();
            writeln!(&mut self.src_file, "        &{}_BITSET_INDEX_CHUNKS[0][0],", uppercase).unwrap();
            writeln!(&mut self.src_file, "        sizeof({0}_BITSET_INDEX_CHUNKS[0])/sizeof({0}_BITSET_INDEX_CHUNKS[0][0]),", uppercase).unwrap();
            writeln!(&mut self.src_file, "        {}_BITSET_CANONICAL,", uppercase).unwrap();
            writeln!(&mut self.src_file, "        sizeof({0}_BITSET_CANONICAL)/sizeof({0}_BITSET_CANONICAL[0]),", uppercase).unwrap();
            writeln!(&mut self.src_file, "        {}_BITSET_MAPPING", uppercase).unwrap();
            writeln!(&mut self.src_file, "    );").unwrap();
            writeln!(&mut self.src_file, "}}").unwrap();
        };
        x();
    ...
    fn emit_chunk_map(
        ...
        impl Debug for Chunk<'_> {
            fn fmt(&self, f: &mut Formatter<'_>) -> fmt::Result {
                let mut string = String::new();
                write!(&mut string, "{:?}", self.chunk).unwrap();
        ...
        for chunk in compressed_words.chunks(chunk_length) {
            chunk_indices.push(chunk_map[&Chunk { chunk }]);
        }

        writeln!(
            &mut self.src_file,
            "static const FimoU8 {}_BITSET_CHUNKS_MAP[{}] = {{{}}};",
            property.to_uppercase(),
            chunk_indices.len(),
        ...
        writeln!(
            &mut self.src_file,
            "static const FimoU8 {}_BITSET_INDEX_CHUNKS[{}][{}]= {{{}}};",
            property.to_uppercase(),
            chunks.len(),
            chunk_length,
...
pub fn emit_codepoints(emitter: &mut RawEmitter, ranges: &[Range<u32>], property: &str) {
    emitter.blank_line();

    let mut bitset = emitter.clone();
    let bitset_ok = bitset.emit_bitset(ranges, property).is_ok();

    let mut skiplist = emitter.clone();
    skiplist.emit_skiplist(ranges, property);
